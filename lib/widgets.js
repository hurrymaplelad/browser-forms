...
var createSelector = function (attributeName) {
    return function (name, parent) {
        var selectorName = parent != null ? parent + "\\[" + name + "\\]" : name;
        return '['+attributeName+'='+selectorName+']';
    }
};
...
mixIntoWidget('email', attachableToInput);
mixIntoWidget('tel', attachableToInput);
...
mixIntoWidget('date', attachableToInput);
...
after(widgets, 'multipleCheckbox', function (widget) {
  ...
  mix(widget, {
    ...
    selector: createSelector('data-name'),
